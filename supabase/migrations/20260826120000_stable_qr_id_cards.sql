-- Stable, reprint-safe QR attendance cards.
-- QR values remain random opaque secrets, validated by the existing one-way hash.
-- The raw value is kept in Supabase Vault so an authorised card operator can
-- reprint the same card without exposing the secret through ordinary list APIs.

alter table public.student_cards
  add column if not exists qr_secret_id uuid;
alter table public.staff_cards
  add column if not exists qr_secret_id uuid;

comment on column public.student_cards.qr_secret_id is
  'Supabase Vault secret id for the permanent QR token; never the raw token.';
comment on column public.staff_cards.qr_secret_id is
  'Supabase Vault secret id for the permanent QR token; never the raw token.';

create unique index if not exists student_cards_one_active_qr_per_person
  on public.student_cards(student_id)
  where card_type = 'qr' and status in ('active','pending');

create unique index if not exists staff_cards_one_active_qr_per_person
  on public.staff_cards(staff_id)
  where card_type = 'qr' and status in ('active','pending');
...
begin
  ...
  where (i.id = v_card_id or i.source_credential_id = v_card_id)
    and i.person_type = v_person_type
