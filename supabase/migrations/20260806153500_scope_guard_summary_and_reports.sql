-- Tighten school-wide summary/report access for class and personal roles.
alter function public.attendance_universal_admin_read_api(text,text,text,jsonb)
  rename to attendance_universal_admin_read_api_scoped_v1;
...
begin
  ...
  if p_action='summary' and not v_is_global then
    ...
    return jsonb_build_object('ok',false,'code','ADMIN_PERMISSION_DENIED');
  elsif p_action='corrections' and not (v_is_global or 'corrections.review'=any(v_permissions)) then
    ...
    if not (v_is_global or 'reports.read'=any(v_permissions)) then return jsonb_build_object('ok',false,'code','ADMIN_PERMISSION_DENIED'); end if;
...
revoke all on function public.attendance_universal_admin_read_api_scoped_v1(text,text,text,jsonb) from public,anon,authenticated;
